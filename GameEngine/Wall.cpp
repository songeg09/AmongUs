...
void Wall::ResolveCircle(Collider* _other)
{
	...
	if (Actor* actor = dynamic_cast<Actor*>(_other->GetTarget()))
	{
		Vector2 MoveVec = actor->GetForce();
		...
		if (dist < radius && dot1 * dot2 <= 0.0f)
		{
			...
			Vector2 NewActorPos = actor->GetPosition() + (m_vec2WallNormal * penetration);
			...
			// 새 위치 적용
			actor->SetPosition(NewActorPos + slideVec * 0.3f);
		}
	}
}
